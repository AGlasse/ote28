...
if process_cv3:
    ...
    for obs_tag in cv3_list:
        ...
        xlim = [350, 650]
        ylim = [350, 650]
        plot.display(image, title, fmin=-0.01, fmax=0.03, xlim=xlim, ylim=ylim, skip=True)
        ...
        col_profile_list = photom.find_profiles(col_identifier, u_vals, v_coadd, image, bstars, col_profile_list,
                                                axis='col', normal='sum')
        eef_list = photom.find_eefs(radii, image, bstars, eef_list)
    plot.plot_eef_list(radii, eef_list)
    ...
    row_profile = photom.combine_profiles(row_profile_list)
    plot.plot_profile(row_profile, code_list, 'row', plot_fit=True)
    col_profile = photom.combine_profiles(col_profile_list)
    #plot.plot_profiles([col_profile], code_list, 'col')
...
# Read in stack of OTE 28 images
if process_ote28:
    ...
    for obs_tag in obs_tags[0:3]:
        ...
        title = file.split('/')[2] + ' - bgd ' + obs_tag
        plot.display(bkg, title, vmin=0.5, vmax=1.0)
        plot.histogram(bkg[450:550, 450:550], title, bins=50)
        print('Mean background = {:10.3f}', np.mean(bkg))
        image_list.append(img)

    image_stack = np.array(image_list)
    stack_bkg = np.nanmedian(image_stack, axis=0)
    title = file.split('/')[2] + ' - stack bgd'
    plot.display(stack_bkg, title, vmin=0.0, vmax=2.0)
    plot.histogram(stack_bkg[450:550, 450:550], title, bins=50)
    print('Mean stacked background = {"10.3f}', np.mean(stack_bkg))
    ...
    for image in image_list:
        ...
        stars = photom.find_sources(obs, bkg_sigma)
...
#    ref_image_file = '../data/' + 'cruciform_sim/' + 'MIRI_FM_MIRIMAGE_F560W_PSF_07.02.00.fits'
    ...
    for bstar in ref_bstars:
        id, x_o, y_o, flux = bstar['id'], bstar['x_0'], bstar['y_0'], bstar['flux_0']
